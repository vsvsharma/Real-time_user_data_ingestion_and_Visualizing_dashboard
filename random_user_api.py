...
def kafka_consumer(ti):
    ...
    try:
        for message in consumer:
            ...
            result[0]["p_date"]=timestamp_formatted
...
def process_picture(ti):
    ...
    picture_df[["uuid", "profile_pic_large","profile_pic_medium",
                "profile_pic_thumbnail"]] = picture_df[["uuid", "profile_pic_large",
                                                         "profile_pic_medium",
                                                         "profile_pic_thumbnail"]].astype(str)
    print(f"The length of picture df is: {len(picture_df)}")
    pd.set_option('display.max_columns', None)
    print(picture_df.dtypes)
    print(picture_df.head())
...
# Populate the list with formatted queries without executing them
            for row in data_values:
                formatted_query = cursor.mogrify(query, row).decode('utf-8')
...
clear_xcom_extracted= PostgresOperator(task_id='clear_xcom_extract',
                             postgres_conn_id='airflow_db_postgres',
                             sql="delete from xcom where key='extracted_data' or key='varun'",
                             dag=dag)
clear_xcom_dataframes=PostgresOperator(task_id='clear_xcom_dataframes',
                             postgres_conn_id='airflow_db_postgres',
                             sql="delete from xcom where key in ('person_data', 'address_data', 'picture_data','login_data', 'loc_tz_data', 'reg_data', 'encrypt_data')",
                             dag=dag)
...
"""
extract_data_operator >> personal_data_operator >> clear_xcom_extracted >> insert_into_db_operator >> clear_xcom_dataframes
extract_data_operator >> address_data_operator >> clear_xcom_extracted >> insert_into_db_operator >> clear_xcom_dataframes
extract_data_operator >> picture_data_operator >> clear_xcom_extracted >> insert_into_db_operator >> clear_xcom_dataframes
extract_data_operator >> login_data_operator >> clear_xcom_extracted >> insert_into_db_operator >> clear_xcom_dataframes
extract_data_operator >> login_data_operator >> clear_xcom_extracted >> insert_into_db_operator >> clear_xcom_dataframes
extract_data_operator >> loc_tz_operator >> clear_xcom_extracted >> insert_into_db_operator >> clear_xcom_dataframes
extract_data_operator >> encrypted_details_operaotor >> clear_xcom_extracted >> insert_into_db_operator >> clear_xcom_dataframes
extract_data_operator >> registration_operator >> clear_xcom_extracted >> insert_into_db_operator >> clear_xcom_dataframes
